...
module.exports.friendsList = async (req, res) => {
  const getFriendsList = await friendRequestModel
    .find({
      $or: [{ senderId: req.user._id }, { sentTo: req.user._id }],
      accepted: true,
      active: true,
    })
    .select("-senderId -accepted -__v -sentTo")
    .populate("senderDetails sentToUserDetails", "_id name");

  const newlist = getFriendsList.map((objs) => {
    if (objs.sentToUserDetails.id.toString() !== req.user._id.toString()) {
      return objs.sentToUserDetails;
    } else return objs.senderDetails;
  });

  return res.json(utils.createSuccessResponse(msgConstants.allFriendsList, newlist));
};
